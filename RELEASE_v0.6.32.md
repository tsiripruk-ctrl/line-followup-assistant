...
- `README.md` — v0.6.32 release notes.

## Database migration
None. v0.6.32 reuses the v0.6.30 fields:
- `progress_summary`
- `waiting_on`
- `next_action`
- `last_progress_at`
...
2. Upload/commit the v0.6.32 source over the existing repository.
3. Do not change current environment variables.
4. Deploy latest commit on Render.
5. Wait for build/deploy success.
6. Open `/health` and verify `version = 0.6.32` and the new health flags.
...
## Rollback
Deploy the previous v0.6.30 commit/tag. There is no v0.6.32 database migration to reverse.
...
- If a task's structured progress snapshot is already wrong from historical data, v0.6.32 will faithfully use that snapshot. Correct/clear contaminated legacy snapshots before relying on them for high-trust reminders.
